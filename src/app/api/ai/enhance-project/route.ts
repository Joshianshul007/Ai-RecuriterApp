...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[AI_ENHANCE_PROJECT] Input:", rawInput.substring(0, 100));
    const result = await enhanceProject(rawInput, existingSkills || []);
    console.log("[AI_ENHANCE_PROJECT] Success");

    return NextResponse.json({ success: true, data: result }, { status: 200 });
  } catch (error: any) {
    console.error("[AI_ENHANCE_PROJECT] Error:", error?.message || error);
    return NextResponse.json(
      {
        success: false,
